...
formSearch.addEventListener('submit', async (e) => {
    ...
    if(textSearch.length > 2) {
        ...
        const haad = document.createElement('h1');
        haad.classList.add('found__item')
        haad.insertAdjacentHTML('afterbegin', `
        class="found">${data.length} вакансий &laquo;<span class="found__item">${textSearch}</span>&raquo;
        `);
        return haad;
    } else {
