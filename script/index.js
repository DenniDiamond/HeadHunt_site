...
const decLoOfNum = (n, titles) =>  n + ' ' + titles[n % 10 === 1 && n % 100 !== 11 ?
...
employerElem.textContent = employment;
...
skillsListElem.classList.add('skills__list');
 ...
 for (const skill of skills) {
    const skillsItemElem = document.querySelector('li');
    skillsItemElem.classList.add('skills__item');
    skillsElem.textContent = skill;
...
formSearch.addEventListener('submit', async (e) => {
    ...
    if(textSearch.length > 2) {
        ...
        renderCards(data);
        found.innerHTML = `${decLoOfNum(data.length, ['вакансия', 'вакансии', 'вакансий'])} вакансий &laquo;${textSearch}&raquo;`;
        formSearch.reset();
    } else {
